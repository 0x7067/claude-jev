...
- **Live route (out-of-band).** Needs `TYPESAFE_API_KEY` on a machine that can call Jev. Omit model and run the Explore prompt above. Exit code `0`; if confidence ≥ 0.75, stdout is JSON containing `updatedInput.model` in `haiku|sonnet|opus`.
