...
1. The Jev answers (intent, confidence, scope, needs_repo) in one line.
2. A one-sentence handling plan implied by that classification.
3. If intent confidence is below 0.55 or intent is `unclear`: the single most
   useful clarifying question to ask the user instead.
