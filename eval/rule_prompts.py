...
EVIDENCE = (
    "Judge only the supplied evidence. Identify the changed construct "
...
def questions(case):
    ...
    return {
        ...
        "applicable": {
            "type": "noul",
            "instructions": f"Does the changed code fall within this rule's scope, including its exceptions: {case['rule']}?",
        },
        "evidence": {
            "type": "noul",
            "instructions": f"Does the supplied material contain the evidence needed to decide compliance with this rule, including any approval history or external definitions it requires: {case['rule']}?",
        },
        "violation": {
            "type": "noul",
    ...
    with output.open("x") as f:
        for repeat in range(args.repeats):
            for case in cases:
                ...
                record = dict(
                    case=case, repeat=repeat, questions=qs, answers=answers, version=jev.version()
                )
