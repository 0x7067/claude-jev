...
- The rules eval now measures the AskUserQuestion answers the way the hook sends them. Its corpora carried only the typed request, so no rerun could see the v0.20.0 answers feature: `extract` records each edit's answers since its request, and `run` composes the request through the same `request_with_answers` the hook calls. Records without answers compose to the old request text, so their cache entries still hit. The rules decision log rows gained a `user_answers` count, so live outcomes can split by whether the user answered questions. `scripts/check_no_stubs.py` joins the Verify contract: `jev.ask` is assigned only in the `cached_ask` wiring, so a probe cannot fake the client and present the result as verification. The fresh corpus has 1,569 real edits, 5 of them carrying answers. Judged live through the real client, those five reached the judge with their picked options (a313bcf9#932, "rspec check in CI failed", among them) and none acted; the full rerun reproduced the prior run's every number (2/248 real-edit blocks at 0.81 and 0.83, 19/29 violations blocked, 0/24 compliant), its seed-0 sample containing none of the five.
